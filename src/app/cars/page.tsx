          ...
          <tbody>
            {cars.map((c: CarRow) => (
              <tr key={c.id} className="odd:bg-white even:bg-gray-50">
                ...
                <td className={`p-2 border ${profits[c.id] == null ? '' : (profits[c.id] as number) > 0 ? 'text-green-700 bg-green-50' : (profits[c.id] as number) < 0 ? 'text-red-700 bg-red-50' : 'text-gray-700 bg-gray-50'}`}>{fmtAED(profits[c.id] as number | null)}</td>
